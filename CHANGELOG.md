...
- `mesChart` (Economies of Scale card 4) now generated from `lrac()` — the
  U-shaped LRAC with economies/MES/diseconomies regions and the MES output
  marked. Replaces the hand-tuned bezier (+ its mobile variant).
- Audit sweep: every U-shaped cost/firm diagram that is actually displayed in
  Theme 3 is now generated by the chart engine. The remaining hand-rolled keys
  (shortRunCostStack, profitsShutdownDiagram, monopolyDwl, mcLrTangency,
  lracEnvelope, etc.) are unused legacy (0 topics); the revenue-topic charts are
  straight-line AR/MR / bar / formula panels, not wobbly curves. `sw.js` →
  `econos-v492`.
