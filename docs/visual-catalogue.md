...
- `mcDemandComparison` — price taker (PC, flat AR=MR=D) vs price maker (MC, gentle downward AR) twin demand panels (3.4.3 C2). **Responsive**: desktop shows the side-by-side wide twin (`ECONOS_MC_DEMAND_COMPARISON_SPEC`); on mobile (≤680px) a `.mss`-style media toggle swaps in single-panel `ECONOS_MC_DEMAND_PC_SPEC` / `ECONOS_MC_DEMAND_MC_SPEC` stacked vertically so neither panel is clipped. Reuse this wide-twin→stacked-single pattern for any side-by-side panel pair that must survive a phone.
