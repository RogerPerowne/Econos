(function () {
  ...
  window.ECONOS_QUIZ = {
    ...
    questions: [
      ...
      /* 8 — multi_select: evaluation of price controls */
      { type: 'multi_select',
        stem: 'A student is writing a 25-mark essay on price controls as a method of protecting consumers. Which of the following statements demonstrate <strong>evaluation</strong> &#8212; as opposed to description or one-sided analysis?',
        opts: [
          'Price ceilings are set below equilibrium and create a shortage because quantity demanded exceeds quantity supplied at the controlled price.',
          'The UK energy price cap was justified as a short-run emergency measure when the price spike reflected an acute, temporary supply shock; but it became unsustainable once higher prices reflected a structural supply constraint, because the redistribution of costs to suppliers and taxpayers cannot substitute for the long-run investment signal that higher market prices would have provided.',
          'A rent ceiling reduces rents for existing tenants, so it is an effective and unambiguously good policy for housing affordability.',
          'Means-tested income transfers are a more efficient alternative to price ceilings when the policy goal is protecting low-income households: they target the benefit to those who need it, preserve the market price signal (incentivising supply and conservation), and avoid the deadweight loss and black market risks that price ceilings create &#8212; though they require administrative capacity that ceilings do not.',
          'Price controls reduce consumer prices and are therefore good for consumers in both the short run and the long run.',
          'The minimum wage floor may not create the unemployment predicted by the competitive model if the labour market has monopsony characteristics &#8212; meaning the textbook prediction of a labour surplus is context-dependent rather than a universal result.'
        ],
        correct: [1, 3, 5],
        exp: '<strong>Option A</strong> is description of a mechanism &#8212; it is accurate but contains no evaluative move: no weighing of trade-offs, no conditional reasoning, no direction about when the policy helps or harms. <strong>Option B</strong> is strong evaluation: it identifies the condition under which the cap is justified (acute, temporary shock) and the condition under which it fails (structural constraint), using the investment signal mechanism to give a specific reason for that distinction. This is a qualified, evidence-grounded conclusion. <strong>Option C</strong> is one-sided analysis that ignores the unintended consequences (landlords exiting, quality deterioration, tenure lock-in) &#8212; a common student error of describing the intended effect as if it were the whole story. <strong>Option D</strong> is evaluation: it names a superior alternative instrument, identifies three specific advantages (targeting, signal preservation, no DWL), and qualifies the comparison with an administrative capacity constraint. This is the evaluative structure of weighing alternatives. <strong>Option E</strong> is one-sided overclaiming &#8212; it ignores the long-run unintended consequences that make ceilings harmful (reduced supply, black markets). <strong>Option F</strong> is evaluation: it introduces the monopsony caveat to qualify the standard competitive model prediction, showing awareness that the textbook result is model-dependent rather than universal.'
      },
