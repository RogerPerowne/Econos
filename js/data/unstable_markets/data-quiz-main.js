(function () {
  ...
  window.ECONOS_QUIZ = {
    ...
    questions: [
      ...
      /* 8 — multi_select: evaluation of buffer stock schemes */
      { type: 'multi_select',
        stem: 'A student is writing a 25-mark essay on <strong>buffer stock schemes as a method of stabilising agricultural commodity prices</strong>. Which of the following statements demonstrate <strong>evaluation</strong> &mdash; as opposed to description or one-sided analysis?',
        opts: [
          'Buffer stock schemes buy surplus commodities when prices are low and sell reserves when prices are high, stabilising the price band',
          'The effectiveness of a buffer stock scheme depends critically on setting the floor and ceiling around the true long-run equilibrium price: if the floor is set above equilibrium, the authority chronically buys more than it sells, accumulating unsustainable surpluses &#8212; precisely the failure mechanism of the International Tin Agreement (1985), which collapsed with a ~50% price crash when funds were exhausted',
          'Buffer stock schemes always benefit both farmers and consumers by reducing price volatility, so they should be adopted for all agricultural commodities',
          'Commodity futures markets provide a private-sector alternative that achieves price certainty for individual producers through hedging contracts, without requiring public funds or physical storage &#8212; making them a more financially sustainable stabilisation mechanism, though they transfer rather than eliminate risk and are inaccessible to smallholder farmers in developing countries',
          'Buffer stocks are a good idea in theory but often fail in practice',
          'Storage costs represent a significant ongoing drain on the buffer stock authority\'s resources, meaning the net welfare gain from price stabilisation must be weighed against the real resource cost of physical storage &#8212; a calculation that will favour buffer stocks more for durable, low-spoilage commodities (metals, grains) than for perishables'
        ],
        correct: [1, 3, 5],
        exp: '<strong>Option B (index 1)</strong> is strong evaluation: it identifies the precise failure condition (floor above long-run equilibrium), links it mechanically to the failure outcome (chronic buying, fund exhaustion), and uses a specific quantified historical example (Tin Agreement 1985, 50% price crash) to reach a directional conclusion about when buffer stocks fail. This is evaluation through conditional analysis grounded in evidence. <strong>Option D (index 3)</strong> is strong evaluation: it introduces an alternative mechanism (futures markets), identifies its advantage (financially sustainable, no public funds), but then qualifies the comparison by naming two specific limitations (risk transfer not elimination; inaccessibility to smallholders). This is a balanced evaluation that avoids one-sided advocacy of either instrument. <strong>Option F (index 5)</strong> is strong evaluation: it identifies storage costs as a quantifiable cost that must be weighed against benefits, and draws a directional implication &#8212; buffer stocks are more cost-effective for durable than perishable commodities. This conditional conclusion is evidence-based and avoids universal claims. <strong>Option A (index 0)</strong> is a distractor: it accurately describes the mechanism of a buffer stock scheme but makes no evaluative move &#8212; no conditions, no weighing, no directional conclusion. <strong>Option C (index 2)</strong> is a distractor: an overclaim that ignores storage costs, financial sustainability constraints, the setting of the price band, and the historical record of scheme failures. <strong>Option E (index 4)</strong> is a distractor: it acknowledges a tension but without any mechanism, named example, or directional argument &#8212; "often fail in practice" is a conclusion with no supporting analysis.'
      },
