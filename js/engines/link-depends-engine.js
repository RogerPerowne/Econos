...
(function () {
  ...
  window.bootLinkDepends = function () {
    if (Progress.getLinkUnlocked() < 2) { TopicLoader.go(TopicLoader.buildUrl('link_context.html')); return; }

    var I    = window.ECONOS_ICONS;
    var DATA = window.ECONOS_LINK_DEPENDS;

    /* Shuffle factor order once at boot so model-ranking factors aren't always first */
    ...
    DATA.scenarios.forEach(function (sc) { shuffle(sc.factors); });
    ...
    function attachHandlers() {
      ...
      if (finishBtn) finishBtn.addEventListener('click', function () {
        ...
        try {
          ...
          Progress.setLinkUnlocked(Math.max(u, 3));
        } catch (e) {}
